...
RUN useradd --system -u "${USER_ID}" -g "${USER_GROUP_ID}" --create-home --home-dir "${USER_HOME}" --no-log-init "${USER}" \
    && echo '[ ! -z "${TERM}" -a -r /etc/motd ] && cat /etc/motd' >> /etc/bash.bashrc; echo "${MOTD}" > /etc/motd


# copy init script to user home
COPY --chown=${USER}:${USER_GROUP} docker-entrypoint.sh ${USER_HOME}/
RUN chmod -R g+rx /home/${USER}/docker-entrypoint.sh
...
RUN chmod g+rwX ${WSO2_SERVER_HOME}/repository/components/lib/mysql-connector.jar
...
COPY --chown=${USER}:${USER_GROUP} am#sample#pizzashack#v1.war ${WSO2_SERVER_HOME}/repository/deployment/server/webapps/
